...
#' @importFrom dplyr %>% pull tibble mutate mutate_all select arrange summarise group_by left_join everything
...
dyRank_data <- function(
  ...
  ## create Y vector
  Y <- dat_tmp %>%
    group_by(id_driver, id_match, id_time, id_rank) %>%
    nest()

  ## compute max(rank) to replace the raw ranking
  ## this is necessary when multiple ranks are recorded for a player/teams
  dat_tmp <- dat_tmp %>% select(-outcome) %>%
    left_join(
      dat_tmp %>% group_by(id_driver, id_match, id_rank) %>%
        summarise(outcome = max(outcome)),
        by = c("id_driver", "id_match", "id_rank")
    ) %>%
  ...
  ## update Y vector
  Y <- left_join(Y,
      group_by(dat_tmp, .data$id_rank, .data$id_match) %>%
        summarise(max_rank = max(.data$outcome)),
        by = c("id_match", "id_rank")
    )
  Yvec <- map2(Y$data, Y$max_rank, function(x, y) {
    tmp <- rep(0, y); tmp[x$outcome] <-1; return(tmp)
  })

  Y <- dplyr::bind_cols(Y, tibble(Yvec))
